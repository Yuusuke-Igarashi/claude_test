...
// region ("world" | "japan") でモードを切り替える。クライアント版
// (public/collector-client.js) と同じソース構成・同じスキーマ。

import { geocode as geocodeNet, isValidCoord } from './geocoder.js';
...
// 都道府県の代表座標（気象警報の配置用。geocoder の内蔵辞書と重複するが自己完結のため保持）
const PREF_COORD = {
...
async function fetchJson(url, { signal } = {}) {
  const res = await fetch(url, {
    signal,
    headers: { 'User-Agent': 'disaster-info-map/1.0', Accept: 'application/json' },
  });
...
// --- 世界版ソース ---
async function collectUSGS(signal, japanOnly) {
  const data = await fetchJson('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.geojson', { signal });
  const out = [];
  for (const f of data.features || []) {
    ...
    out.push({
      ...
      description: `震源: ${p.place || '不明'} / マグニチュード ${p.mag ?? '?'} / 深さ ${f.geometry?.coordinates?.[2] ?? '?'} km`,
      place: p.place || '', lat, lon, magnitude: p.mag ?? null,
...
async function collectGDACS(signal, japanOnly) {
  const data = await fetchJson('https://www.gdacs.org/gdacsapi/api/events/geteventlist/MAP', { signal });
  const out = [];
  for (const f of data.features || []) {
    const [lon, lat] = f.geometry?.coordinates || [];
    if (!isValidCoord(lat, lon)) continue;
...
async function collectP2PQuake(signal) {
  const data = await fetchJson('https://api.p2pquake.net/v2/history?codes=551&limit=30', { signal });
  const out = [];
  for (const item of Array.isArray(data) ? data : []) {
    ...
    if (!isValidCoord(lat, lon)) {
      const g = PREF_COORD[h.name] || (await geocodeNet(h.name));
...
// --- 日本版・追加ソース ---
async function collectJMAQuake(signal) {
  const data = await fetchJson('https://www.jma.go.jp/bosai/quake/data/list.json', { signal });
  const out = [];
  for (const e of (Array.isArray(data) ? data : []).slice(0, 30)) {
    let coords = parseCod(e.cod) || PREF_COORD[e.anm] || (e.anm ? await geocodeNet(e.anm) : null);
    if (!coords) continue;
    const maxi = e.maxi || '';
...
async function collectP2PTsunami(signal) {
  const data = await fetchJson('https://api.p2pquake.net/v2/history?codes=552&limit=10', { signal });
  const out = [];
  ...
  for (const item of Array.isArray(data) ? data : []) {
    ...
    for (const a of item.areas || []) {
      const g = PREF_COORD[a.name] || (await geocodeNet(a.name));
      if (!g || seen.has(a.name)) continue;
      seen.add(a.name);
      out.push({
        ...
        description: `津波予報: ${a.name || ''} / ${a.grade || ''}${a.maxHeight?.description ? ' / ' + a.maxHeight.description : ''}`,
...
async function collectJMAWarnings(signal) {
  const codes = Object.keys(JMA_OFFICES);
  const settled = await Promise.allSettled(
    codes.map((code) =>
      fetchJson(`https://www.jma.go.jp/bosai/warning/data/warning/${code}.json`, { signal }).then((data) => ({ code, data }))
    )
  );
  const out = [];
...
/**
 * すべてのソースを収集・正規化して返す。
 * @param {object} opts { region: "world"|"japan", signal }
 */
export async function collectAll({ signal, region = 'japan' } = {}) {
  const isJapan = region === 'japan';
  const tasks = isJapan
    ? [
        { name: '気象庁(地震)', fn: () => collectJMAQuake(signal) },
        { name: '気象庁(気象警報)', fn: () => collectJMAWarnings(signal) },
        { name: 'P2P地震情報', fn: () => collectP2PQuake(signal) },
        { name: 'P2P地震情報(津波)', fn: () => collectP2PTsunami(signal) },
        { name: 'USGS', fn: () => collectUSGS(signal, true) },
        { name: 'GDACS', fn: () => collectGDACS(signal, true) },
      ]
    : [
        { name: 'USGS', fn: () => collectUSGS(signal, false) },
        { name: 'P2P地震情報', fn: () => collectP2PQuake(signal) },
        { name: 'GDACS', fn: () => collectGDACS(signal, false) },
      ];
