...
- **AIレポート生成** — 詳細パネルの「📄 AIレポート生成」から、そのイベントについて
  **Web検索で最新の関連情報を収集し、Claude(LLM)が日本語レポートを自動作成**（参照元リンク付き）
  - Anthropic の [web_search ツール](https://platform.claude.com/)でClaude自身が検索・要約・引用
  - APIキーを安全に扱うため**バックエンドが必要**。**Nodeサーバー版**、または
    公開サイト向けに **Cloudflare Worker**（`worker/`）を用意すれば利用可
...
3. 既定ブランチへ push すると `.github/workflows/static.yml` が `public/` を自動デプロイ
...
**公開サイトでAIレポート生成も使いたい場合**は、`worker/` の Cloudflare Worker を
デプロイし、発行URLを `public/config.js` の `reportApiUrl` に設定してください
（手順は [`worker/README.md`](worker/README.md)）。APIキーは Worker のシークレットとして
保持されるため、ブラウザには露出しません。
...
地図・災害情報の収集だけなら **依存ゼロ**（`node server.js` のみ）で動きます。

#### AIレポート生成を使う場合（任意）

「📄 AIレポート生成」を使うには、Anthropic のSDKとAPIキーが必要です:

```bash
npm install                                  # @anthropic-ai/sdk を導入
ANTHROPIC_API_KEY=sk-ant-... node server.js   # APIキーを渡して起動
# 使用モデルを変えたい場合: ANTHROPIC_MODEL=claude-sonnet-5 など（既定 claude-opus-4-8）
```

> APIキー未設定でも地図・災害情報は通常どおり動作し、レポート生成時のみ案内が表示されます。
> レポートはWeb検索結果に基づくAI生成物です。内容の正確性は保証されないため、必ず公式情報をご確認ください。
...
| `src/report.js` | AIレポート生成（web_search + Claude、`@anthropic-ai/sdk`） |
...
| `public/config.js` | フロント設定（AIレポートのバックエンドURL 等） |
| `src/report-core.js` | レポートの共通ロジック（Node版/Worker版で共有） |
| `worker/` | 公開サイト用 AIレポートバックエンド（Cloudflare Worker） |
| `.github/workflows/static.yml` | `public/` を GitHub Pages に自動デプロイ |
...
`POST /api/report` — 1件のイベントについてAIレポートを生成します（サーバー版のみ）。

```jsonc
// リクエスト
{ "event": { "type": "earthquake", "title": "...", "place": "福島県沖", "lat": 37.5, "lon": 141.5, "time": "..." } }

// レスポンス
{
  "markdown": "## 概要\n...",                       // 生成されたレポート(Markdown)
  "sources": [{ "url": "https://...", "title": "..." }], // 参照した情報源
  "model": "claude-opus-4-8",
  "generatedAt": "2026-07-15T00:00:00.000Z"
}
```

主なエラー: `503 NO_API_KEY`（APIキー未設定）/ `501 SDK_NOT_INSTALLED`（`npm install` 未実行）。
